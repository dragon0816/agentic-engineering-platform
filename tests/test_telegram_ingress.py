...
from common.assets import SECRET_PATTERN, reject_embedded_secrets
...
def test_config_refuses_a_pasted_token_duplicates_and_a_plain_origin() -> None:
    ...
    with pytest.raises(ValidationError):
        TelegramIngressConfig.model_validate({**config().model_dump(), "bot_token": TOKEN})
    with pytest.raises(ValidationError):
        config(
    ...
    # The token's shape is now credential material everywhere in the repository.
...
def test_an_unmapped_sender_is_refused_before_its_text_is_read(tmp_path: Path) -> None:
    transport = FakeTransport([[update(1, STRANGER, "shipment.run")]])
    item, agent, runner = ingress(tmp_path, transport)
    result = poll(item)
    assert result.failure is None
    assert [d.disposition for d in result.deliveries] == ["unmapped_sender"]
    assert result.deliveries[0].outcome is None
    assert runner.bridge.events == () and agent.runs() == ()
    assert transport.sent() == [result.deliveries[0].reply]
...
def test_a_mapped_sender_whose_actor_is_not_bound_is_refused_by_the_agent(tmp_path: Path) -> None:
    ...
    assert "company_owner_required" in delivery.reply
    assert runner.bridge.events == () and agent.runs() == ()
...
def test_help_and_status_are_answered_locally_without_a_run(tmp_path: Path) -> None:
    transport = FakeTransport(
        [
            [update(1, OWNER, "/help")],
            [update(2, OWNER, "shipment.run")],
            [update(3, OWNER, "/status")],
        ]
    )
    item, agent, runner = ingress(tmp_path, transport)
    first = poll(item)
    assert first.deliveries[0].disposition == "answered" and first.deliveries[0].reply == HELP
    assert runner.bridge.events == ()
    poll(item)
    third = poll(item)
    assert third.deliveries[0].disposition == "answered"
    assert "1 run(s)" in third.deliveries[0].reply and "succeeded" in third.deliveries[0].reply
...
def test_a_transport_error_is_reported_without_the_token(tmp_path: Path) -> None:
    error = OSError(f"connection refused to https://api.telegram.org/bot{TOKEN}/getUpdates")
    ...
    assert TOKEN not in result.failure.message and "[redacted]" in result.failure.message
...
def test_a_conflict_stops_the_loop(tmp_path: Path) -> None:
    ...
    stop = asyncio.Event()
    failure = asyncio.run(item.run(stop, interval_s=0.01))
    assert failure is not None and failure.code == "telegram_conflict"
    # A server error is retryable and the loop keeps going until asked to stop.
    item, _, _ = ingress(tmp_path / "b", FakeTransport(status=503))
    result = poll(item)
    assert result.failure is not None and result.failure.code == "telegram_http_error"
...
def test_a_redelivered_update_is_handled_once_and_the_batch_is_confirmed(tmp_path: Path) -> None:
    ...
    polls = [payload for url, payload in transport.calls if url.endswith("/getUpdates")]
...
def test_replies_are_chunked_and_a_failed_reply_is_recorded_not_raised(tmp_path: Path) -> None:
