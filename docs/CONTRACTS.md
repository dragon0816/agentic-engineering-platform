...
`organization` are for any member of the platform, `team` is for members of
the owning group, and `private` is for nobody else. A user-owned asset marked
`team` names no team to check, so it stays with its owner.
