...
class _RunRecord:
    ...
    def __init__(
        ...
        *,
        ...
        self.sequence = 0
        # Present only when the engine journals this run; the run's place in it.
...
class WorkflowEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self.bridge = bridge
        # Without a journal the engine behaves exactly as before: in memory only.
    ...
    async def execute(
        ...
        *,
        ...
        if idempotency_key is not None:
            ...
            signature = intent_signature(target, checked, run_arguments)
    ...
    def _journal_failure(self, record: _RunRecord, index: int, error: CheckpointStoreError) -> None:
        record.status = "failed"
        record.failure = Failure(
            code=f"checkpoint_{error.code}", message="Run evidence could not be recorded"
        )
    ...
    async def recover(
        ...
        *,
        ...
        """Continue a suspended journalled run, in this process or a later one.

        The stored manifest is provenance: the same version must still be
        installed and identical. Every remaining step is authorized again, the
        ...
        if parent.status != "suspended":
            return self._rejected(target, checked, "run_not_suspended", "needs_input")
        installed = self.workflows.get(target)
        if installed is None:
            return self._rejected(target, checked, "workflow_not_installed")
        if installed != parent.manifest:
            # Provenance, not authority: a changed definition is a new decision.
            return self._rejected(target, checked, "manifest_changed", "needs_input")
        plan = parent.recovery_plan()
        start = plan.next_step
        if start is None:
            return self._rejected(target, checked, "run_complete")
        try:
        ...
        if rejection is not None:
            return self._rejected(target, checked, *rejection)
        if plan.steps[start].state == "uncertain":
            binding = self.bridge.installed.get(_target(installed.steps[start]))
            replayable = chosen.uncertain == "replay_side_effects" or (
                chosen.uncertain == "replay_read_only"
                and binding is not None
                and binding.spec.side_effect == "read"
            ...
            if not replayable:
                return self._rejected(target, checked, "uncertain_side_effect", "needs_input")
        if not self._replay_allowed(checked, installed, start):
            return self._rejected(target, checked, "permission_denied", "failed")
        ...
        resumed.step_results = [
            CapabilityResult(
                trace=checked.trace, status="succeeded", data=data, handler_invoked=True
            )
        ...
        return await self._launch(
            ...
            timeout_seconds,
            start=start,
            opening=f"recover run {parent.run_id} from step {start + 1}",
        )
